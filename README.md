...
- ♥ **お気に入り管理** — ハートボタンで作品をお気に入り登録、フィルターで絞り込み
...
**Q: お気に入りが保存されない**
- A: ファイルモード（`file://`）では localStorage に保存されます。`npm run serve-start` でサーバーモード起動するとディスクに永続化されます。
...
- **お気に入りフィルター**: ♥ ハートボタンで登録・「♥ お気に入りのみ」フィルターで絞り込み（サーバーモード時はディスク保存、ファイルモード時は localStorage）
