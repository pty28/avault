...
/**
 * dmm-library.jsonの作品に対して、DMM APIから女優名、メーカー情報を取得
 */
...
// --force フラグをチェック（既にfetchedでも強制取得）
...
if (!API_ID || !AFFILIATE_ID) {
  ...
  console.log('\n💡 使用方法:');
  console.log('   DMM_API_ID=your_api_id DMM_AFFILIATES_ID=your_affiliate_id npm run fetch-info');
  console.log('\nまたは、環境変数を設定してください:');
...
async function fetchInfoForItem(item, index, total) {
  // 必要なフィールドを初期化
  ...
  if (!item.hasOwnProperty('makerName')) {
    item.makerName = '';
  }

  if (!item.hasOwnProperty('makerId')) {
    item.makerId = '';
  }
  ...
  if (!item.hasOwnProperty('labelId')) {
    item.labelId = '';
  }
  ...
  try {
    ...
    if (result.result && result.result.status === 200 && result.result.items && result.result.items.length > 0) {
      ...
      // メーカー情報を抽出（iteminfo.maker は配列）
      const makerName = (apiItem.iteminfo && apiItem.iteminfo.maker && apiItem.iteminfo.maker.length > 0)
        ? apiItem.iteminfo.maker[0].name
        : '';
      const makerId = (apiItem.iteminfo && apiItem.iteminfo.maker && apiItem.iteminfo.maker.length > 0)
        ? apiItem.iteminfo.maker[0].id
        : '';

      // レーベル情報を抽出（iteminfo.label は配列）
      const labelName = (apiItem.iteminfo && apiItem.iteminfo.label && apiItem.iteminfo.label.length > 0)
        ? apiItem.iteminfo.label[0].name
        : '';
      const labelId = (apiItem.iteminfo && apiItem.iteminfo.label && apiItem.iteminfo.label.length > 0)
        ? apiItem.iteminfo.label[0].id
        : '';
      ...
      if (makerName) {
        console.log(`   📦 メーカー: ${makerName}`);
      }

      if (labelName) {
        console.log(`   🏷️  レーベル: ${labelName}`);
      }
...
async function main() {
  console.log('🚀 作品情報取得スクリプトを開始します\n');

  if (forceMode) {
    console.log('⚠️  --force モード: 既に取得済みのデータも強制的に再取得します\n');
  }
  ...
  try {
    ...
    const alreadyHasInfo = data.filter(item => item.actresses && item.actresses.length > 0);

    // needsFetch は --force モードかどうかで判定ロジックが変わる
    let needsFetch;
    if (forceMode) {
      // --force モード: 作品コードがあれば、女優情報やisFetchedの有無に関わらず全て取得対象
      needsFetch = data.filter(item => item.productCode);
    } else {
      // 通常モード: 既にfetchedなら対象外
      needsFetch = data.filter(item =>
        item.productCode &&
    ...
    if (forceMode) {
      console.log(`   処理対象（--force モード）: ${needsFetch.length}件（女優情報の有無に関わらず）\n`);
    } else {
      console.log(`   女優情報あり（スキップ）: ${alreadyHasInfo.length}件`);
      console.log(`   取得が必要: ${needsFetch.length}件\n`);
    }

    if (needsFetch.length === 0 && !forceMode) {
      console.log('✨ すべての作品に女優情報が既に存在します！');
      return;
    }

    console.log('📥 DMM APIから情報を取得しています...\n');
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // 通常モード: 既に女優情報がある場合はスキップ（--force モードではスキップしない）
      if (!forceMode && item.actresses && item.actresses.length > 0) {
        console.log(`[${i + 1}/${data.length}] ⏭️  スキップ: ${item.title.substring(0, 40)}... (女優情報あり)`);
        updatedItems.push(item);
        continue;
      }

      // 通常モード: 既にAPI取得済み（isFetched = true）の場合はスキップ
      if (!forceMode && item.hasOwnProperty('isFetched') && item.isFetched === true) {
        console.log(`[${i + 1}/${data.length}] ⏭️  スキップ: ${item.title.substring(0, 40)}... (取得済み)`);
        updatedItems.push(item);
        continue;
      }

      // APIから取得（作品コードがない場合もスキップ）
      if (!item.productCode) {
        updatedItems.push(item);
        continue;
      }
      ...
      // APIから取得
      const updatedItem = await fetchInfoForItem(item, i, data.length);
    ...
    const withMaker = updatedItems.filter(item => item.makerName);
    const withLabel = updatedItems.filter(item => item.labelName);

    console.log('\n✅ 完了！\n');
    console.log('📊 統計:');
    console.log(`   総作品数: ${updatedItems.length}件`);
    console.log(`   女優情報あり: ${withActresses.length}件`);
    console.log(`   メーカー情報あり: ${withMaker.length}件`);
    console.log(`   レーベル情報あり: ${withLabel.length}件`);
