...
3. 該当女優が見つかると、別名欄の下に「素人総合Wikiで開く ↗」リンクが表示され、`/details` が返す `page_url`（seesaawiki の女優ページ）を新規タブで開ける
4. 必要に応じて別名欄を手動編集
5. 「入力」ボタンで別名群を検索ボックスへスペース区切りで取り込み、即時フィルタ適用（ページ先頭へスクロール）。「キャンセル」または背景クリックで閉じる
